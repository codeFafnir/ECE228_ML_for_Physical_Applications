...
def _build_sign_vectors(d: int, n_blocks: int, seed: int,
    ...
    Uses CPU generator for reproducibility across devices. Results cached by (d, n_blocks, seed).
    """
    key = (d, n_blocks, seed)
    if key not in _sign_vector_cache:
        ...
        for _ in range(n_blocks):
            ...
            signs.append(s)
        _sign_vector_cache[key] = signs
    cached = _sign_vector_cache[key]
    return [s.to(device) for s in cached]
